...
export default function CustomerView({
  ...
  const [customSelectedAddOns, setCustomSelectedAddOns] = useState<{ name: string; price: number }[]>([]);
  ...
  const handleAddToCart = (
    ...
    const addOnKey = selectedAddOns.map(a => `${a.name}-${a.price}`).join('-');
    const cartItemId = `${item.id}-${addOnKey}-${specialInstructions || ''}`;
  ...
  const handleCheckout = async () => {
    ...
    // Check if deliveryAddress is blank
    if (!deliveryAddress.trim()) {
      alert("Please provide a valid Delivery Destination address.");
      return;
    }
    ...
    try {
      ...
      if (paymentMethod === 'Cash on Delivery') {
        ...
        // Direct Wallet Payment confirmation prompt in modal!
        setPaymentStep('provider_select');
        setShowPaymentModal(true);
      } else {
        // Direct credit card checkout input page for Paystack / Flutterwave!
        setPaymentStep('card_details');
        setShowPaymentModal(true);
      }
    } catch (e: any) {
      console.error(e);
      alert(`Failed to place your order: ${e.message || e}`);
    } finally {
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-5 sticky top-24 shadow-sm flex flex-col max-h-[calc(100vh-140px)]">
                ...
                <div className="flex-1 overflow-y-auto py-3 space-y-3">
                  {cart.length === 0 ? (
                    ...
                    cart.map((item) => {
                      const singleItemTotal = item.price + (item.selectedAddOns || []).reduce((sum, ad) => sum + ad.price, 0);
                      const itemLineTotal = singleItemTotal * item.quantity;
                      return (
                        <div key={item.id} className="flex justify-between items-start text-xs border-b border-gray-100 pb-3">
                          <div className="max-w-[70%] space-y-0.5">
                            ...
                            {item.specialInstructions && (
                              <div className="text-[10px] text-gray-500 italic font-medium bg-slate-50 border border-slate-100 rounded px-1.5 py-0.5 mt-1 inline-block">
                                "{item.specialInstructions}"
                              </div>
                            )}

                            <div className="flex items-center gap-1.5 mt-1">
                              <span className="text-[10px] text-[#FF6B35] font-extrabold font-mono">₦{singleItemTotal.toLocaleString()} each</span>
                              {item.quantity > 1 && (
                                <span className="text-[9px] text-gray-400 font-bold font-mono">(₦{itemLineTotal.toLocaleString()} total)</span>
                              )}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="p-1 rounded bg-gray-100 hover:bg-gray-200 text-gray-600 cursor-pointer"
                            >
                ...
                {cart.length > 0 && (
                  <div className="border-t border-gray-100 pt-3 space-y-3.5 text-xs">
                    ...
                    <button
                      ...
                      className={`w-full py-3 bg-[#FF6B35] text-white font-black rounded-xl text-xs hover:bg-[#E55A2B] transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer ${
                        isPlacingOrder ? 'opacity-85 cursor-not-allowed' : ''
                      }`}
                    >
                      {isPlacingOrder ? (
                        <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setCardNumber('5061 1234 5678 9010');
                        setCardExpiry('12/29');
                        setCardCVV('123');
                        setPaymentError(null);
                      }}
                      className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-black rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer border border-slate-200"
                    >
                      💳 Quick Fill Demo Card (Verve)
                    </button>
          ...
          <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100 text-slate-800 flex flex-col transform transition-all">
            ...
            <div className="relative h-40 shrink-0">
              ...
              <button
                ...
                className="absolute top-4 right-4 text-white hover:text-gray-200 text-sm font-bold bg-black/40 w-7 h-7 rounded-full flex items-center justify-center cursor-pointer transition-colors"
              >
                ✕
              </button>
              <div className="absolute bottom-4 left-6 right-6">
                <span className="text-[9px] font-black uppercase tracking-wider bg-[#FF6B35] text-white px-2.5 py-1 rounded-full">
                  Customize Order
                </span>
                <h3 className="text-lg font-black text-white mt-1.5 tracking-tight">{customizingItem.name}</h3>
              </div>
            </div>

            {/* Modal Body */}
            <div className="p-6 space-y-5 max-h-[50vh] overflow-y-auto">
              <p className="text-xs text-gray-500 font-medium leading-relaxed">{customizingItem.description}</p>
              
              {/* Base Price display */}
              <div className="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                <span className="text-xs font-bold text-gray-600">Base Price</span>
                <span className="text-sm font-black text-emerald-800">₦{customizingItem.price.toLocaleString()}</span>
              </div>
              ...
              {customizingItem.addOns && customizingItem.addOns.length > 0 && (
                <div className="space-y-2.5">
                  <h4 className="text-xs font-black text-gray-700 uppercase tracking-wider flex items-center justify-between">
                    <span>Optional Add-ons / Extras</span>
                    <span className="text-[10px] font-bold text-gray-400 normal-case">Select multiple</span>
                  </h4>
                  <div className="space-y-1.5">
                    {customizingItem.addOns.map((addOn, index) => {
                      const isSelected = customSelectedAddOns.some(a => a.name === addOn.name);
                      return (
                        <label
                          key={index}
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <input
                              ...
                              onChange={() => {
                                if (isSelected) {
                                  setCustomSelectedAddOns(customSelectedAddOns.filter(a => a.name !== addOn.name));
                                } else {
                                  setCustomSelectedAddOns([...customSelectedAddOns, addOn]);
                                }
                              }}
                          ...
                          <span className="text-xs font-extrabold text-emerald-800 font-mono">+₦{addOn.price.toLocaleString()}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              {/* Special Instructions */}
              <div className="space-y-1.5">
                <label className="text-xs font-black text-gray-700 uppercase tracking-wider">Special Instructions</label>
                <textarea
                  placeholder="e.g. No onions, make it spicy, extra utensils..."
                  value={customSpecialInstructions}
                  onChange={(e) => setCustomSpecialInstructions(e.target.value)}
                  rows={2}
                  className="w-full bg-white border border-gray-200 rounded-xl px-3.5 py-2 text-xs font-semibold text-gray-700 outline-none focus:border-[#FF6B35]/40 shadow-3xs resize-none"
                />
              </div>
              ...
              <div className="flex justify-between items-center border-t border-gray-100 pt-4">
                ...
                <div className="flex items-center gap-3 bg-gray-100 p-1 rounded-xl">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-white hover:bg-gray-50 flex items-center justify-center text-gray-600 font-black text-sm shadow-3xs cursor-pointer"
                  >
                    -
                  </button>
                  <span className="text-xs font-extrabold text-gray-700 px-2 min-w-[20px] text-center">{customQuantity}</span>
                  <button
                    type="button"
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
              <div className="text-left">
                <span className="text-[10px] font-bold text-gray-400 block uppercase tracking-wider">Total Price</span>
                <span className="text-base font-black text-[#FF6B35]">
                  ₦{(
                    (customizingItem.price + customSelectedAddOns.reduce((sum, a) => sum + a.price, 0)) *
                    customQuantity
                  ).toLocaleString()}
                </span>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setCustomizingItem(null)}
                  className="px-4 py-2.5 bg-white border border-gray-200 text-gray-700 text-xs font-bold rounded-xl hover:bg-gray-50 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={() => {
                    handleAddToCart(customizingItem, customSelectedAddOns, customSpecialInstructions, customQuantity);
                    setCustomizingItem(null);
                  }}
                  className="px-5 py-2.5 bg-emerald-800 hover:bg-emerald-950 text-white text-xs font-black rounded-xl transition-all shadow-md flex items-center gap-1 cursor-pointer"
                >
                  Confirm Add to Cart 🛒
                </button>
              </div>
      ...
      {cart.length > 0 && customerTab === 'home' && (
        <div className="lg:hidden fixed bottom-6 left-4 right-4 z-40 animate-bounce">
          <button
            ...
            className="w-full bg-[#FF6B35] hover:bg-[#E55A2B] text-white py-3.5 px-5 rounded-2xl shadow-xl flex items-center justify-between text-xs font-black transition-all active:scale-[0.98] cursor-pointer"
          >
            <div className="flex items-center gap-2">
              <span className="bg-white/20 text-white w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px]">
          ...
          <div className="w-full max-w-md bg-white h-full flex flex-col p-6 shadow-2xl relative">
            ...
            <div className="flex items-center justify-between border-b border-gray-100 pb-4 mb-4">
              <h3 className="text-sm font-black text-gray-800 flex items-center gap-2">
                ...
                <span>Your FoodHub Basket</span>
              </h3>
              <button
                onClick={() => setShowMobileCart(false)}
                className="text-gray-400 hover:text-gray-600 font-bold bg-gray-100 w-8 h-8 rounded-full flex items-center justify-center cursor-pointer transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              {cart.length === 0 ? (
                <div className="text-center py-16 text-gray-400 flex flex-col items-center justify-center">
                  ...
                  <p className="text-[10px] mt-1 text-gray-400">Add delicious items to get started!</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Cart Items List */}
                  <div className="space-y-3.5">
                    {cart.map((item) => {
                      ...
                      const itemLineTotal = singleItemTotal * item.quantity;
                      return (
                        <div key={item.id} className="flex justify-between items-start text-xs border-b border-gray-100 pb-3.5">
                          <div className="max-w-[65%] space-y-0.5">
                            ...
                            <div className="flex items-center gap-1.5 mt-1">
                              <span className="text-[10px] text-[#FF6B35] font-extrabold font-mono">₦{singleItemTotal.toLocaleString()}</span>
                              {item.quantity > 1 && (
                                <span className="text-[9px] text-gray-400 font-bold font-mono">(₦{itemLineTotal.toLocaleString()} total)</span>
                              )}
                            </div>
                  ...
                  {/* Checkout inputs (Destination, Tip, Payment) */}
                  <div className="space-y-4 pt-1">
                    {/* Destination */}
                    <div className="bg-gray-50 border border-gray-100 p-3.5 rounded-2xl space-y-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs">📍</span>
                        <span className="text-[10px] font-black text-gray-500 uppercase tracking-wider font-mono">Delivery Destination</span>
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          value={deliveryAddress}
                          onChange={(e) => setDeliveryAddress(e.target.value)}
                          className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-[11px] font-semibold text-gray-700 outline-none focus:border-emerald-500 shadow-3xs"
                        />
                      </div>
                    ...
                    {/* Tip Rider */}
                    <div className="flex items-center justify-between bg-gray-50 border border-gray-100 p-2.5 rounded-xl text-xs">
                      <span className="text-[10px] font-bold text-gray-500">🏍️ Tip Delivery Rider</span>
                      <div className="flex gap-1.5">
                        {[0, 500, 1000].map((tip) => (
                          <button
                    ...
                    <div className="bg-gray-50 border border-gray-100 p-3.5 rounded-2xl space-y-2">
                      <span className="text-[10px] font-black text-gray-500 block uppercase tracking-wider font-mono">💳 Checkout Payment Option</span>
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          onClick={() => { setPaymentMethod('Card'); setPaymentProvider('Paystack'); }}
                          className={`px-2 py-2.5 text-[10px] font-black rounded-xl border text-center transition-all cursor-pointer ${
                            paymentProvider === 'Paystack' && paymentMethod === 'Card'
                              ? 'bg-teal-600 text-white border-teal-600 shadow-sm'
                    ...
                    {/* Cost breakdown */}
                    <div className="space-y-1.5 text-[11px] text-gray-500 border-t border-gray-150 pt-3">
                      <div className="flex justify-between">
                        <span>Subtotal</span>
                        <span>₦{getCartSubtotal().toLocaleString()}</span>
                      </div>
              ...
              <div className="border-t border-gray-100 pt-4 mt-2">
                <button
                  onClick={async () => {
                    ...
                    // Close only after attempting checkout
                    setShowMobileCart(false);
                  }}
                  disabled={isPlacingOrder}
                  className="w-full py-3.5 bg-[#FF6B35] text-white font-black rounded-2xl text-xs hover:bg-[#E55A2B] transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isPlacingOrder ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    ...
                    <>
                      <CreditCard className="w-4.5 h-4.5" />
                      <span>Place Food Order (₦{getCartTotal().toLocaleString()})</span>
                    </>
                  )}
                </button>
              </div>
